...
02/26/26 updated to terminate if negative probability density is detected
         updated to correct for trace at the level of differential equation for DMB
         updated for numerical precision in density and wave function 
         
03/02 updated the mass output, corrected the current output to me, and added in a.u. 
      added analytical_only relaxation mode option      
      implemented Bose-Einstein also through "thermal_weight": "bose_einstein",
...
def compute_psi_t(energy, phi, c_n0, t_fs, gamma_rescale,intrinsic_dynamics=True):
    """
    ...
    h_eVfs = 4.135667696604003 # 2*np.pi*hbar_eVfs
 #   E_int = np.max(energy) - np.min(energy)
 #   gamma_n=(energy-energy[0])**2/E_int/h_eVfs 
 
 
  #  gamma_n=gamma_rescale*(energy-energy[0])/h_eVfs 
    
    num_states = len(energy)
    gamma_n = np.zeros(num_states)

    for n in range(num_states):

        gamma_eff = 0.0

        for m in range(n):   # only lower-energy states

            deltaE = energy[n] - energy[m]

           # gamma_eff += np.abs(deltaE) / h_eVfs
            gamma_eff += deltaE / h_eVfs

        gamma_n[n] = gamma_rescale * gamma_eff # factor of 2 is to be consistent with LO
    ...
    omega_n_prime=omega_n-1j*gamma_n
...
def rel_term(rho_t_loc, energy_loc, phi, Temp, relaxation_model,gamma_rescale,tol_rho, thermal_weight):
    """
...
#    hbar = 0.6582119569  # eV·fs
    ...
    beta = 1.0/(k_B*Temp)
    h_eVfs = 4.135667696604003 # 2*np.pi*hbar_eVfs
    gamma_n = gamma_rescale*(energy_loc - energy_loc[0]) / h_eVfs # using h instead of hbar now
#    E_int = np.max(energy_loc) - np.min(energy_loc)
#    gamma_n=gamma_rescale*(energy_loc-energy_loc[0])**2/E_int/h_eVfs     
    num_states = len(energy_loc)
    
    # Transform to energy eigenbasis
    rho_eig = phi.conj().T @ rho_t_loc @ phi
    #rho_eig[np.abs(rho_eig) < tol_rho] = 0.0
    idx = np.diag_indices_from(rho_eig)
    rho_eig[idx] = np.where(np.abs(rho_eig[idx]) < tol_rho, 0.0, rho_eig[idx])
    rho_eig /= np.trace(rho_eig)  
    ...
    if relaxation_model == "DMB":
        ...
        deltaE = energy_loc - energy_loc[0]
    
        # ---------------------------------------------------------
        # Build transition matrix gamma_mat[n,k]
        # = rate FROM state k TO state n
        # ---------------------------------------------------------
        ...
        for n in range(num_states):
            ...
            for k in range(n + 1, num_states):
                ...
                dE = energy_loc[k] - energy_loc[n]
    
                omega = np.abs(dE) / h_eVfs
    
                if thermal_weight == "boltzmann":
    
                    # downward relaxation
                    gamma_mat[n, k] = omega
    
                    # upward thermal excitation
                    gamma_mat[k, n] = omega * np.exp(-beta * dE)
    
                elif thermal_weight == "bose_einstein":
    
                    if abs(dE) < 1e-12:
                        nB = 0.0
                    else:
                        nB = 1.0 / (np.exp(beta * dE) - 1.0)
    
                    # absorption
                    gamma_mat[k, n] = omega * nB
    
                    # emission
                    gamma_mat[n, k] = omega * (nB + 1.0)
    
                elif thermal_weight == "zero_weight" or thermal_weight is None:
    
                    gamma_mat[n, k] = omega
    
                else:
                    raise ValueError(
                        f"Unknown thermal_weight option: {thermal_weight}"
                    )
    
        gamma_mat *= 2*gamma_rescale # factor of 2 is to be consistent with LO
    
        # ---------------------------------------------------------
        # Total outflow rate from each state
        # ---------------------------------------------------------
        ...
        for n in range(num_states):
            ...
            delta_pop = rho_eig[n, n] - rho_eq_eig[n, n]
    
            trace_corr += gamma_out[n] * delta_pop
        ...
        for n in range(num_states):
            ...
            relax_term = -gamma_out[n] * delta_pop
    
            d_rho_eig[n, n] = (
                relax_term
                + rho_eig[n, n] * trace_corr
            )
  ...
  #      exp_terms = np.exp(-beta * (energy_loc - energy_loc[0]))
        ...
        if thermal_weight == "boltzmann":
            exp_terms = np.exp(-beta * deltaE)

        elif thermal_weight == "bose_einstein":
            exp_terms = np.zeros_like(deltaE)
            for i in range(len(deltaE)):
                if abs(deltaE[i]) > 1e-12:
                    exp_terms[i] = 1.0 / (np.exp(beta * deltaE[i]) - 1.0)
                else:
                        exp_terms[i] = 0.0

        else:
            raise ValueError(f"Unknown thermal_weight: {thermal_weight}")
        ...
        for n in range(num_states):
            for k in range(n+1, num_states):  # only upper triangle
                ...
                #val = np.abs(deltaE)/h_eVfs #(np.abs(deltaE) / hbar)**2
                
                if thermal_weight == "bose_einstein":
                
                    if abs(deltaE) < 1e-12:
                        nB = 0.0
                    else:
                        nB = 1.0 / (np.exp(beta * deltaE) - 1.0)
                ...
                elif thermal_weight == "boltzmann":
                
                    gamma_mat[n, k] = omega 
                    gamma_mat[k, n] = omega * np.exp(-beta * deltaE)
                
                elif thermal_weight == "zero_weight" or thermal_weight is None:
                    raise ValueError(
                        "thermal_weight='zero_weight' is not supported for the LO relaxation model. "
                        "Use 'boltzmann' or 'bose_einstein'."
                            )
                
                else:
                    raise ValueError(f"Unknown thermal_weight option: {thermal_weight}")              
             
                
                
 # #               val = deltaE**2/E_int/h_eVfs #(np.abs(deltaE) / hbar)**2
 #                val = np.abs(deltaE)/h_eVfs #(np.abs(deltaE) / hbar)**2
...
def project_density_matrix(rho_loc, phi_loc,tol_rho):
    """
 ...
 #   rho_proj[np.abs(rho_proj) < tol_rho] = 0.0 # zero out numerical garbage
...
def initialize_state(x_int, current_phi, conv_P2x, P_cen, delta_P,tol_cn, tol_rho):
    ...
    rho_0 = project_density_matrix(rho_0, current_phi,tol_rho)
...
def read_json_parameters(json_filename):
    ...
    params_out = {
        ...
        "thermal_weight": params.get("thermal_weight", "boltzmann"),   
...
thermal_weight = json_params["thermal_weight"]
gamma_rescale = json_params["gamma_rescale"]
integrator = json_params["integrator"] 
analytical_only = (relaxation_model == "analytical_only")
use_analytical_model = True  # keep analytical enabled
if thermal_weight == "zero_weight" and relaxation_model != "DMB":
    raise ValueError(
        f"thermal_weight='zero_weight' is only allowed with relaxation_model='DMB'. "
        f"Current model: {relaxation_model}"
    )
...
#tol_cn = len(x_int) * np.finfo(np.float64).eps #1e-14
tol_cn = np.sqrt(len(x_int)) * np.finfo(np.float64).eps #1e-14
tol_rho = 10.0*tol_cn #*rho_t.shape[0] * np.finfo(np.float64).eps # tolerance for numerical garbage in rho

psi_0, c_n0, rho_0 = initialize_state(x_int, current_phi, conv_P2x, P_cen, delta_P,tol_cn,tol_rho) 
psi_t = np.copy(psi_0) # time dependent state vector for analytical solution of Schrod eq.

if not analytical_only:
    tol_neg_dens =  -1e-4
    # Initialize density matrix
#    psi_0, c_n0, rho_0 = initialize_state(x_int, current_phi, conv_P2x, P_cen, delta_P,tol_cn) 
    rho_t = rho_0.astype(np.complex128)
    # Compute gamma_n just for output
    h_eVfs = 4.135667696604003 # 2*np.pi*hbar_eVfs
    E_int = np.max(current_energy ) - np.min(current_energy )
    gamma_n=(current_energy -current_energy[0])**2/E_int/h_eVfs 
...
#    gamma_n = (current_energy - current_energy[0]) / hbar_eVfs
    ...
    rel = rel_term(rho_t, current_energy, current_phi, Temp,relaxation_model,gamma_rescale,tol_rho, thermal_weight)[1:-1, 1:-1]
    rv1=np.zeros_like(rel); rv2=np.zeros_like(rel)
else:
    # Analytical-only mode: skip density matrix
    rho_t = None
    tau_min = tau_max = 0.0
...
#rel = rel_term(rho_t, current_energy, current_phi, Temp,relaxation_model,gamma_rescale,tol_rho)[1:-1, 1:-1]
...
# Physical conversion
me_per_amu = 1822.888486209  # 1 amu in units of electron mass

# Append to parameter section
if mass_from_h5 is not None:
    mass_me = mass_from_h5
    mass_amu = mass_me / me_per_amu

    output_file.write(f"#   mass = {mass_me:.6f} m_e\n")
    output_file.write(f"#          = {mass_amu:.6f} amu\n")
else:
    output_file.write(f"#   mass = Not found in all_simulation_data.h5\n")    
...
if analytical_only:
    output_file.write("# Columns:\n")
    output_file.write("#   time [fs], field_target [kV/cm], P_analytical [C/m²]\n")
else:
    output_file.write("# Columns:\n")
    output_file.write("#   time [fs], field_target [kV/cm], P_analytical [C/m²], P_density_matrix [C/m²], Tr[rho]\n")    
output_file.write("# ------------------------------------------------------------------------------------------------------\n")
...
for n_t in range(nsteps):
   ...
   # min_diag = np.min(np.real(np.diag(rho_t)))    
    ...
    if not analytical_only:
        ...
        print(
            f"Step: {n_t:6d} | "
            f"Target Field: {E_current:10.4f} kV/cm | "
            f"Actual Field: {fields[field_idx]:10.4f} kV/cm | "
            f"Trace ρ: {trace_val:.6f}"
        )
        
        # --- Physicality checks ---
        if trace_val < 0.0 or trace_val > 1.001:
            raise ValueError(
                f"FATAL ERROR at step {n_t}: "
                f"Trace of density matrix is unphysical "
                f"(Trace ρ = {trace_val:.6f}, expected ∈ [0, 1])."
            )
        

        
        if min_diag < tol_neg_dens:
            raise ValueError(
                f"FATAL ERROR1 at step {n_t}: negative probability detected "
                f"on density matrix diagonal (min rho_ii = {min_diag:.6e}, "
                f"tolerance = {tol_neg_dens:.6e})."
            )
    else:
        print(
            f"Step: {n_t:6d} | "
 ...
 #       print("here")
        ...
        # ground_state_split = abs(current_energy[1] - current_energy[0])
 
        # if ground_state_split < 1e-4:
        #    print(
        #    f"Skipping field {fields[field_idx]:.6f} kV/cm "
        #    f"due to ground-state degeneracy "
        #    f"(ΔE = {ground_state_split:.3e} eV)"
        #    )
        ...
        if not analytical_only:
            rho_t = project_density_matrix(rho_t, current_phi,tol_rho)
  #          rho_t[np.abs(rho_t) < tol_rho] = 0.0 # zero out numerical garbage
...
#                use_analytical_model = False
    ...
    if use_analytical_model:
        psi_t = compute_psi_t(current_energy, current_phi, c_n0, t_fs_psi,gamma_rescale,intrinsic_dynamics)
    ...
    if not analytical_only:
        ...
        diag = np.real(np.diag(rho_t))
        min_diag = np.min(np.real(diag)) 
#        min_diag = np.min(np.real(np.diag(rho_t)))    
        if min_diag < tol_neg_dens:
            raise ValueError(
                f"FATAL ERROR2 at step {n_t}: negative probability detected "
                f"on density matrix diagonal (min rho_ii = {min_diag:.6e}, "
                f"tolerance = {tol_neg_dens:.6e})."
            )    
        rho_t[np.diag_indices_from(rho_t)] = np.maximum(diag, 0.0) #set negative elements to zero to elemenate
        rho_t /= np.trace(rho_t)    
 #       print("psi",np.max(psi_t2))
 #       print("rho",np.max(np.abs(rho_t)))
    ...
    if not analytical_only:
        ...
        output_file.write(f"{n_t * delta_t:.6f} {E_current:.6f} {x_expect[n_t] / conv_P2x:.6f}\n")
...
# plot probability density for all two approaches for comparison    
    ...
    if json_params["plot_mode"] == "screen" and n_t % n_out == 0:
        ...
        plt.title(
            f'Step={n_t:6d} | t={t_fs:8.1f} fs | E={E_current:.0f} kV/cm | T={Temp:.0f} K'
)
        
#        plt.title(
#            f'Current step = {n_t}, time = {t_fs:.2f} fs, E-field = {E_current:.0f} kV/cm'
#        )
