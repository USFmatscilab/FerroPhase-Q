...
Updates
02/26 updated to re-shift potentia to zero after interpolation.
      updated to output frequency difference so it can be compared with QHO
03/04  fixed bug in frequency conversion      
...
Vol = params['Vol']
mass_amu = params['mass']   # input mass in amu
mass = mass_amu * au2e      # convert to electron masses
#mass = params['mass']
...
# # ---- Combine and save both original and interpolated data ----
# max_len = max(len(x), len(x_int))

# out = np.full((max_len, 4), np.nan)
# out[:len(x), 0] = x
# out[:len(x), 1] = v
# out[:len(x_int), 2] = x_int
# out[:len(x_int), 3] = v_int

# np.savetxt(
#     'potential_comparison.txt',
#     out,
#     header='x_orig(nm)  V_orig_shifted(eV)  x_interp(nm)  V_interp(eV)',
#     fmt='%.10f'
# )
...
def analyze_field_data_by_value(field_value_input, n_states_to_plot=3):
    ...
    try:
        with h5py.File('all_simulation_data.h5', 'r') as f:
            ...
            print("\nAll eigenvalues (index, eV, 2pi THz, THz, ΔTHz):")
#            ev_to_hz = 2.417989e14  # 1 eV = 2.417989e14 Hz
            prev_freq_thz = None
